									...
									&& pc_checkskill(sd, itAutoheal.skill_id) >= itAutoheal.skill_lv){
										...
										if(last_tick >= itAutoheal.last_use){
											if(unit_skilluse_id(bl, bl->id, itAutoheal.skill_id, itAutoheal.skill_lv)){
												...
												if(sd->aa.skill_cd < (last_tick + skill_get_delay( itAutoheal.skill_id, itAutoheal.skill_lv )) )
													sd->aa.skill_cd = last_tick + skill_get_delay( itAutoheal.skill_id, itAutoheal.skill_lv );
							...
							for(auto &itAutobuffskills : sd->aa.autobuffskills){
								if(last_tick >= itAutobuffskills.last_use){
									if(itAutobuffskills.is_active 
										...
										&& skill_check_condition_castend(sd, itAutobuffskills.skill_id, itAutobuffskills.skill_lv)){
										...
										if(unit_skilluse_id(&sd->bl, sd->bl.id, itAutobuffskills.skill_id, itAutobuffskills.skill_lv)){
											...
											if(sd->aa.skill_cd < (last_tick + skill_get_delay( itAutobuffskills.skill_id, itAutobuffskills.skill_lv ) + skill_get_cast(itAutobuffskills.skill_id, itAutobuffskills.skill_lv)) )
												sd->aa.skill_cd = last_tick + skill_get_delay( itAutobuffskills.skill_id, itAutobuffskills.skill_lv ) + skill_get_cast(itAutobuffskills.skill_id, itAutobuffskills.skill_lv);
										}
									}
								} else
									skip = true;
...
bool aa_check_target(map_session_data *sd, unsigned int id)
{
	...
	if (bl && path_search(NULL, sd->bl.m, sd->bl.x, sd->bl.y, bl->x, bl->y, 1, CELL_CHKNOREACH) && distance_xy(sd->bl.x, sd->bl.y, bl->x, bl->y) < 11){
		...
		if (md && md->status.hp > 0
			&& bosstype == BOSSTYPE_NONE // comment this line to allow it on boss and miniboss
		    ){
