...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	//PK damage rates
	if (sd && bl->type == BL_PC && (battle_config.pk_mode || map_getmapflag(sd->bl.m, MF_PK)) && damage && map_getmapflag(bl->m, MF_PVP));
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ //Non pvp/gvg, check party/guild settings.
		...
		{
			int s_party = status_get_party_id(s_bl);
			if(s_party && s_party == status_get_party_id(t_bl) && !(battle_config.cellpvp_party_enable && map_getcell( t_bl->m, t_bl->x, t_bl->y, CELL_CHKPVP )) )
				state |= BCT_PARTY;
		}
		if( flag&BCT_GUILD || state&BCT_ENEMY )
		{
			int s_guild = status_get_guild_id(s_bl);
			int t_guild = status_get_guild_id(t_bl);
			if(s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && guild_isallied(s_guild, t_guild))) && !(battle_config.cellpvp_guild_enable && map_getcell( t_bl->m, t_bl->x, t_bl->y, CELL_CHKPVP )) )
				state |= BCT_GUILD;
		}
...
// rAthena
    ...
    { "feature.goldpc_maxpoints",			&battle_config.feature_goldpc_maxpoints,        300,    300,    100000,       },
#ifdef RENEWAL
