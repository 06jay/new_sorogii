...
bool pc_isUseitem(map_session_data *sd,int n)
{
	...
	if( itemdb_group.item_exists( IG_MF_NOTELEPORT, nameid ) ){
		if( ( mapdata->getMapFlag(MF_NOTELEPORT) || mapdata_flag_gvg2( mapdata ) ) || map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP ) ) {
	...
	if( itemdb_group.item_exists( IG_MF_NORETURN, nameid ) ){
		if( mapdata->getMapFlag(MF_NORETURN) || map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP ) ){
			return false;
		}
...
int pc_dead(map_session_data *sd,struct block_list *src)
{
	...
	if(battle_config.bone_drop==2
		|| (battle_config.bone_drop==1 && mapdata->getMapFlag(MF_PVP) || map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) && sd->state.pvp) )
	{
